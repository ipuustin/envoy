...
static void freeAsn1Integer(ASN1_INTEGER* integer) {
  ASN1_STRING_free(static_cast<ASN1_STRING*>(integer));
}
#endif
} // namespace

absl::string_view Asn1Utility::cbsToString(Envoy::Extensions::Common::Cbs::CBS& cbs) {
  auto str_head = reinterpret_cast<const char*>(CBS_data(&cbs));
  return {str_head, CBS_len(&cbs)};
}

ParsingResult<absl::optional<Envoy::Extensions::Common::Cbs::CBS>>
Asn1Utility::getOptional(Envoy::Extensions::Common::Cbs::CBS& cbs, unsigned tag) {
  int is_present;
  Envoy::Extensions::Common::Cbs::CBS data;
  if (!CBS_get_optional_asn1(&cbs, &data, &is_present, tag)) {
    return "Failed to parse ASN.1 element tag";
  }

  return is_present ? absl::optional<Envoy::Extensions::Common::Cbs::CBS>(data) : absl::nullopt;
}

ParsingResult<std::string> Asn1Utility::parseOid(Envoy::Extensions::Common::Cbs::CBS& cbs) {
  Envoy::Extensions::Common::Cbs::CBS oid;
...
ParsingResult<Envoy::SystemTime>
Asn1Utility::parseGeneralizedTime(Envoy::Extensions::Common::Cbs::CBS& cbs) {
  Envoy::Extensions::Common::Cbs::CBS elem;
...
ParsingResult<std::string> Asn1Utility::parseInteger(Envoy::Extensions::Common::Cbs::CBS& cbs) {
#if 0
  Envoy::Extensions::Common::Cbs::CBS num;
...
#endif
  // c2i_ASN1_INTEGER was dropped from OpenSSL 1.1.1. Need to convert the above to use
  // d2i_ASN_INTEGER.
  (void)cbs;
  return absl::string_view("Failed to parse ASN.1 INTEGER");
}

ParsingResult<std::vector<uint8_t>>
Asn1Utility::parseOctetString(Envoy::Extensions::Common::Cbs::CBS& cbs) {
  Envoy::Extensions::Common::Cbs::CBS value;
...
ParsingResult<absl::monostate> Asn1Utility::skipOptional(Envoy::Extensions::Common::Cbs::CBS& cbs,
                                                         unsigned tag) {
...
ParsingResult<absl::monostate> Asn1Utility::skip(Envoy::Extensions::Common::Cbs::CBS& cbs,
                                                 unsigned tag) {
