...
bool cbsContainsU16(Envoy::Extensions::Common::Cbs::CBS& cbs, uint16_t n) {
  while (CBS_len(&cbs) > 0) {
    uint16_t v;
...
ContextImpl::ContextImpl(Stats::Scope& scope, const Envoy::Ssl::ContextConfig& config,
  ...
  for (auto& ctx : tls_contexts_) {
    ...
    if (!capabilities_.provides_ciphers_and_curves &&
        !set_strict_cipher_list(ctx.ssl_ctx_.get(), config.cipherSuites().c_str())) {
  ...
  if (!capabilities_.provides_certificates) {
    for (uint32_t i = 0; i < tls_certificates.size(); ++i) {
      ...
      if (private_key_method_provider) {
        ...
        throw EnvoyException(
            fmt::format("BoringSSL private key method isn't supported in the OpenSSL mode"));
      } else {
        // Load private key.
  ...
  for (TlsContext& tls_context : tls_contexts_) {
    STACK_OF(SSL_CIPHER*) ciphers = SSL_CTX_get_ciphers(tls_context.ssl_ctx_.get());
...
void ContextImpl::logHandshake(SSL* ssl) const {
  ...
  if (group > 0) {
    ...
    case NID_secp384r1: {
      incCounter(ssl_curves_, "P-384", unknown_ssl_curve_);
    } break;
    }
  }
...
#define SSL_set_tlsext_host_name_no_warn(s, name)                                                  \
  SSL_ctrl(s, SSL_CTRL_SET_TLSEXT_HOSTNAME, TLSEXT_NAMETYPE_host_name, static_cast<void*>(name))
...
bssl::UniquePtr<SSL> ClientContextImpl::newSsl(const Network::TransportSocketOptions* options) {
  ...
  if (!server_name_indication.empty()) {
    sni_ = server_name_indication;
    // TODO: make sure the ClientContextImpl object lives longer than the ssl pointer.
    // C++17: data() returns a non-const pointer, guarateneed to be null-terminated.
    const char* sni = server_name_indication.c_str();
    int len = strlen(sni);
    char* tmp = static_cast<char*>(malloc(len + 1));
    strcpy(tmp, sni);
    tmp[len] = '\0';
    const int rc = SSL_set_tlsext_host_name_no_warn(ssl_con.get(), tmp);
...
#define SSL_CTX_set_tlsext_ticket_key_cb_no_warn(ssl, cb)                                          \
  SSL_CTX_callback_ctrl(ssl, SSL_CTRL_SET_TLSEXT_TICKET_KEY_CB,                                    \
                        reinterpret_cast<void (*)(void)>(cb))
...
ServerContextImpl::ServerContextImpl(Stats::Scope& scope,
  ...
  if (!config.capabilities().provides_certificates) {
    SSL_CTX_set_cert_cb(
        ...
        +[](SSL* ssl, void*) {
          static_cast<ServerContextImpl*>(SSL_CTX_get_app_data(SSL_get_SSL_CTX(ssl)))
  ...
  for (uint32_t i = 0; i < tls_certificates.size(); ++i) {
    ...
    if (config.disableStatelessSessionResumption()) {
      ...
      SSL_CTX_set_tlsext_ticket_key_cb_no_warn(
          ctx.ssl_ctx_.get(),
...
bool ServerContextImpl::isClientEcdsaCapable(SSL* ssl) {
  ...
  if (TLS1_get_version(ssl) >= TLS1_2_VERSION && tls_max_version_ == TLS1_3_VERSION) {
    ...
    for (int i = 0; i < num_sigalgs; i++) {
      ...
      if (psignhash == NID_ecdsa_with_SHA256) {
        return true;
      }
    }
  }
...
bool ServerContextImpl::isClientOcspCapable(SSL*) {
#if 0
  ...
  return false;
#else
  // TODO: implement using SSL_read_early_data()
  return false;
#endif
}
...
void ServerContextImpl::selectTlsContext(SSL* ssl) {
  const bool client_ecdsa_capable = isClientEcdsaCapable(ssl);
  const bool client_ocsp_capable = isClientOcspCapable(ssl);

  // TODO: return error in case OCSP stapling fails, success at the end
...
void ServerContextImpl::TlsContext::addClientValidationContext(
  ...
  for (;;) {
    ...
    if (sk_X509_NAME_find(list.get(), name)) {
      continue;
    }
